...
// denylist that spreads process.env and drops ONLY the daemon's SLACK_*/CONDOTTO_*
...
const SECRET_PREFIXES = ["SLACK_", "CONDOTTO_"];
...
console.log(`[spike] CONDOTTO_POISON present in scrubbed env?   ${"CONDOTTO_POISON" in scrubbed}`);
...
const condottoLeaked = /condotto=\[CONDOTTO-POISON/.test(line);
...
console.log(`CONDOTTO_POISON LEAKED to the shell:              ${condottoLeaked}  (want false)`);
...
const pass = ranClean && !slackLeaked && !condottoLeaked && toolKept && homeKept && pathKept;
if (pass) {
  console.log("[spike] PASS — denylist scrub keeps SLACK_*/CONDOTTO_* out of the agent shell while the CLI runs and PATH/HOME/toolchain survive. BUILD as denylist.");
  process.exit(0);
}
