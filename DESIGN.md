...
| `tools` | `string[] \| {type:'preset',preset:'claude_code'}` | **Availability** — which built-ins exist for the session at all. Orthogonal to `allowedTools`, and the two are easy to conflate: naming a tool in `allowedTools` also supplies it, so an empty `allowedTools` (what workflows-on uses) leaves only the runtime's *default* set — which on native builds **excludes `Grep` and `Glob`**. The `claude_code` preset value is a no-op, byte-identical to omitting the option; only an explicit list restores them. `disallowedTools` still subtracts from an explicit list, and a name the runtime doesn't expose is ignored rather than an error. Verified 2026-07-26, `scripts/spike-tools.ts`. |
| `allowedTools` | `string[]` | Auto-approve (bare names or `mcp__server__*`) — *not* a way to restrict availability, despite reading like one |
