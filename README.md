...
> Code CLI is one human at one terminal. Conduit is your machine, your
...
for exactly what that means and what Conduit does — and does not — defend against.
...
- [Configure `conduit.toml`](#configure-conduittoml)
- [Run](#run)
...
Conduit is TypeScript run directly by Bun — no build step.
...
git clone https://github.com/chadrem/conduit.git
cd conduit
bun install
```
...
- `conduit` — the compiled daemon (a single `bun build --compile` binary), and
...
next to itself (or wherever `CONDUIT_CLAUDE_CLI` points).

Build the bundle yourself from a clone (produces `dist/<platform>/{conduit,claude}`):
...
> Cross-compiling builds the `conduit` binary for any target, but the `claude`
...
Conduit uses your machine's Claude login — no API key.
...
The daemon scrubs its own `SLACK_*`/`CONDUIT_*` secrets out of the agent's shell,
...
Conduit connects over **Socket Mode** (an outbound WebSocket), so there is **no
...
2. **OAuth & Permissions → Bot Token Scopes** — add:
   ...
   - `commands` — the `/conduit` slash command.
   - `app_mentions:read` — the `@Conduit …` mention path.
   ...
   - `users:read` — resolve user IDs ↔ names (needed for `@Conduit grant @user`).
...
4. **Event Subscriptions** → on (no URL under Socket Mode). Subscribe to bot
   ...
   `app_mention`.
5. **Slash Commands** → create `/conduit` (the "Request URL" field can be any
...
7. **Invite the bot to your channel**: `/invite @Conduit`. Without this, reads
...
> profile → **⋯** → **Copy member ID**. It looks like `U0123ABC`; Conduit writes
...
Conduit reads **one** file: `conduit.toml`. Copy the tracked, commented template
and fill it in:
...
`conduit.toml` is **gitignored** because it holds your Slack tokens. The daemon
discovers `./conduit.toml` by default; override with `--config <path>` or
`CONDUIT_CONFIG`. **Every value can also be set (or overridden) by an environment
...
The one file to know. Read [`conduit.example.toml`](conduit.example.toml) — it's
...
[paths]
db = "conduit.sqlite"                       # audit log, sessions, roles (env: CONDUIT_DB_PATH)
worktrees_root = "~/tmp/conduit-worktrees"  # per-session git worktrees   (env: CONDUIT_WORKTREES_ROOT)

[defaults]                 # used when a repo below sets none
model = "opus"             # opus | sonnet | fable          (env: CONDUIT_DEFAULT_MODEL)
effort = "high"            # low | medium | high | xhigh | max  (env: CONDUIT_DEFAULT_EFFORT)
auto_approve = true        # architects skip their own Approve click (env: CONDUIT_AUTO_APPROVE=off)
cost_cap_usd = 10          # per-thread runaway brake       (env: CONDUIT_COST_CAP_USD)
max_concurrent_turns = 6   # box-wide cap on live turns     (env: CONDUIT_MAX_CONCURRENT_TURNS)

# A repo Conduit can be pointed at. Assign it with `/conduit assign webapp`.
[[repos]]
name = "webapp"
...
land_cmd = "make land"            # architect-ordered `@Conduit land`, run by the daemon (not the agent)
...
A throwaway **`testrepo`** (`~/tmp/conduit-testrepo`, override `CONDUIT_TEST_REPO`)
is registered automatically with echo/no-op land/deploy — the safe target for
your first session. Create it with `git init ~/tmp/conduit-testrepo && (cd
~/tmp/conduit-testrepo && git commit --allow-empty -m init)`, or point a
...
bun start -- --config /etc/conduit/conduit.toml
```

From a binary bundle:

```sh
./conduit                       # discovers ./conduit.toml
./conduit --config /etc/conduit/conduit.toml
./conduit --version             # -> conduit 0.1.0
./conduit --help
```
...
… [daemon] ready — db=…/conduit.sqlite, sessions on record: 0
```
...
1. **Assign a session:** `/conduit assign testrepo`
   Conduit posts an anchor message; its thread is your session. (To assign an
   *existing* thread instead, mention **`@Conduit assign`** inside it — slash
...
4. **Ship it:** `@Conduit land` (and `@Conduit deploy`) run the repo's configured
   command **through the gate** — on `testrepo` those are safe no-ops.
5. **End it:** `@Conduit stop` keeps the worktree for later; `@Conduit stop clean`
...
| Command | Who | Does |
|---|---|---|
| `/conduit assign [repo]` | architect | Start a new session in this channel (defaults to `testrepo`). |
| `/conduit status` | architect | Daemon-wide **operator dashboard** (ephemeral): uptime, session counts, turns-in-flight vs. cap, pending approvals, config summary. |
| `/conduit stop` | anyone | Lists this channel's sessions and points you to the in-thread stop. |
...
sessions themselves, an architect `@Conduit grant`s them architect rights (see
[the trust model](#empowering-domain-experts--grant--auto-approve)).

**In-thread** (mention `@Conduit` inside a session thread):

| Mention | Who | Does |
|---|---|---|
| `@Conduit assign` | architect | Assign *this* thread as a session. |
| `@Conduit status` | anyone | This channel's sessions + their settings. |
| `@Conduit stop [clean]` | architect | End the session; `clean` also discards the worktree. |
| `@Conduit cancel` | architect | Interrupt the running turn (e.g. a runaway workflow); the session lives on. |
| `@Conduit land` / `@Conduit deploy` | architect | Run the repo's ship path (gated; daemon-run). |
| `@Conduit budget <usd>` | architect | Raise this thread's cost ceiling. |
| `@Conduit model <opus\|sonnet\|fable>` | architect | Set the implementer model. |
| `@Conduit effort <low…max>` | architect | Set reasoning effort. |
| `@Conduit subagents on\|off` | architect | Read-only parallel exploration fan-out. |
| `@Conduit workflows on\|off` | architect | Multi-agent Workflow tool (gated + confined). |
| `@Conduit workflows write on\|off` | architect | Let confined workflow/subagent calls write **in the worktree** without a per-write click (out-of-worktree/credentials still refused). |
| `@Conduit ultra on\|off` | architect | Preset: `xhigh` effort + subagents + workflows. |
| `@Conduit auto-approve on\|off` | architect | Run an architect's own turns without the Approve click (on by default). |
| `@Conduit grant @user architect [everywhere]` | architect | Delegate authority (this channel, or `everywhere`). Persists across restarts. |
| `@Conduit revoke @user [everywhere]` | architect | Remove a runtime grant. |
...
- **`@Conduit grant @user architect [everywhere]`** delegates command authority at
  ...
  (while `conduit.toml` stays authoritative for config-defined roles).
  `@Conduit revoke @user` takes it back.
- **`@Conduit auto-approve on|off`** (on by default) lets an **architect's own**
  ...
  **mechanical hard-deny floor still holds** even under auto-approve: out-of-
  ...
  still refused with no button. Dial it off per thread with `@Conduit auto-approve
  off`, or globally with `CONDUIT_AUTO_APPROVE=off`.
...
### What Conduit does **not** defend against

Conduit is for a **small, mutually-trusting team** — a startup's PM plus a couple
...
against. Conduit invests heavily in defense against *mistakes*, *prompt injection
...
- **Linux (systemd):** [`deploy/conduit.service`](deploy/conduit.service). Runs as
  a dedicated `conduit` user, restarts on failure, reads the Claude token from an
  ...
  ```
- **macOS (launchd):** [`deploy/com.conduit.daemon.plist`](deploy/com.conduit.daemon.plist).
...
sqlite3 -header -column conduit.sqlite \
...
> `sqlite3 conduit.sqlite ".backup snapshot.sqlite"` and query the copy.

### Upgrades & schema migrations

Upgrade in place: stop the daemon, swap the `conduit` binary (keep its `claude`
...
interval after an **explicit `@Conduit stop clean`** — it **never** touches a
worktree bound to a live or parked session, so a parked thread always resumes.
Plain `@Conduit stop` keeps the worktree for reactivation; use `stop clean` when
you're done with a thread for good.

### Cost governance

Each thread has a cost ceiling (`cost_cap_usd`); a runaway thread pauses and pings
the architect, who raises it with `@Conduit budget <usd>`. `@Conduit cancel`
...
Back up **`conduit.toml`** (your config + secrets) and **`conduit.sqlite`** (the
...
| Symptom | Fix |
|---|---|
| `not_in_channel` on assign | `/invite @Conduit` into the channel. |
| Boot: `WARNING: no architects configured` | Set `architects` in `conduit.toml` (or `CONDUIT_ARCHITECTS`) and restart. |
| Boot: `Repo "x" missing at …` | Point `[[repos]].path` at a real git repo, or remove the entry. Create the default `~/tmp/conduit-testrepo` or override `CONDUIT_TEST_REPO`. |
| `Configuration error: …` at boot | The message names the bad field. Fix `conduit.toml` (compare against `conduit.example.toml`). |
| Binary: "found no `claude` CLI beside it" | Ship `claude` next to the `conduit` binary, or set `CONDUIT_CLAUDE_CLI` to an installed `claude`. |
| Boot: `default model "x" not in harness models` | Use `opus`, `sonnet`, or `fable` for `[defaults].model`. |
| Approve button does nothing | The clicker isn't an architect. Check `architects` / `@Conduit grant`. |
...
Conduit is intended to ship as open source. Add the `LICENSE` of your choice
before distributing.
