...
- Execute the build plan in DESIGN.md §8. **M0, M1, and M2 are done (2026-07-18).** M0 settled gating = `PreToolUse` `defer`; M1 shipped the Slack echo session (live demo clean); M2 added the policy engine + defer-based approval loop + roles (architect-only assign/stop/approvals, config-authoritative via `CONDUIT_ARCHITECTS`/`conduit.roles.json`). Full facts in DECISIONS.md. **Next up: Milestone 3** (real repo test/land/deploy, streaming status, concurrency, cost budgets, injection hardening, production-data gate). Each milestone is independently demoable.
