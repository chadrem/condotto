...
`DESIGN.md` is the authoritative, self-contained design document — read it before doing anything; this file only orients you.

## Process rules (from DESIGN.md)

- **The build is complete.** `DESIGN.md` is the authoritative, self-contained spec of the system as it stands (§8 is the current-capabilities summary); `DECISIONS.md` is the chronological log of decisions and verified facts. Keep appending to `DECISIONS.md`; update `DESIGN.md` when reality disagrees with it.
